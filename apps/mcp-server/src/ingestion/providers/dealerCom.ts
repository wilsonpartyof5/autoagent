/**
 * Dealer.com Provider Mapper
 * 
 * Normalizes Dealer.com API/feed responses to UVS format.
...
/**
 * Dealer.com vehicle data structure
 * Supports common Dealer.com field variations and naming conventions
 */
...
// Reuse helper functions from Homenet (same logic)
// These helper functions are identical across providers

/**
 * Parse year, make, model from Dealer.com description or title if available
 */
...
/**
 * Derive dealer-provided price from Dealer.com data
 * Tries price/sellingPrice first, then retailPrice, then price_history, then rejects
 */
function deriveDealerPrice(raw: DealerComVehicle): number {
  const listingId = raw.id || raw.vehicleId || raw.vehicle_id || raw.vin || 'unknown';
  
  // Primary: Use selling price or price if available and positive
  const sellingPrice = raw.sellingPrice ?? raw.selling_price;
  ...
  // Secondary: Use retail price if selling price not available
  const retailPrice = raw.retailPrice ?? raw.retail_price;
  if (typeof retailPrice === 'number' && retailPrice > 0) {
    console.warn(JSON.stringify({
      event: 'dealercom_price_from_retail',
      listingId,
      price: retailPrice,
      message: 'Using retail price as dealer-provided price (selling price not available)',
    }));
    return retailPrice;
  }
  ...
  if (priceHistory && Array.isArray(priceHistory) && priceHistory.length > 0) {
    ...
    if (validPrices.length > 0) {
      ...
      if (priceEntry.price) {
        console.warn(JSON.stringify({
          event: 'dealercom_price_from_history',
          listingId,
  ...
  throw new Error(`Missing dealer-provided price for Dealer.com listing ${listingId}. Cannot create UVS record without valid pricing.`);
}

/**
 * Determine availability status from Dealer.com data
 */
function determineAvailability(raw: DealerComVehicle): UVS['availability'] | undefined {
  // Check if price is 0 or missing (might indicate unavailable)
  const price = raw.sellingPrice ?? raw.selling_price ?? raw.price ?? raw.retailPrice ?? raw.retail_price;
...
/**
 * Normalize Dealer.com vehicle to UVS format
 * 
 * @param raw - Raw Dealer.com vehicle data
 * @returns Normalized UVS vehicle
 * @throws Error if required fields (year/make/model/price) cannot be derived
 */
export function normalize(raw: DealerComVehicle): UVS {
  ...
  if (!year || year < 1900 || year > 2100) {
    console.warn(JSON.stringify({
      event: 'dealercom_missing_year',
      listingId: raw.id || raw.vehicleId || raw.vehicle_id || raw.vin,
  ...
  const odometer = miles !== undefined && typeof miles === 'number' && miles >= 0 ? {
    value: miles,
    unit: 'mi' as const, // Dealer.com uses miles by default
  } : undefined;
  ...
  // Parse engine specs from description string
  ...
  // Extract explicit numeric fields
  ...
  // Create or enhance engine specs with explicit fields if available
  // Even if no description string, create engine object from numeric fields
  let finalEngineSpecs = engineSpecs;
  if (!finalEngineSpecs && (cylinders !== undefined || horsepower !== undefined || displacement !== undefined)) {
    // Create engine object from numeric fields even if no description was parsed
    finalEngineSpecs = {};
  }
  ...
  return {
    id: (raw.id as string) || (raw.vehicleId as string) || (raw.vehicle_id as string) || (raw.vin as string) || `dc-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    baseIdentity: {
      ...
      stockNumber: (raw.stockNumber || raw.stock_number || raw.stock) as string | undefined,
      listingId: (raw.id || raw.vehicleId || raw.vehicle_id) as string | undefined,
      vehicleType,
    },
    condition,
    coreSpecs: {
    ...
    marketData,
    operational: {
      dataSource: 'dealer-com',
      lastSyncedAt: now,
      syncStatus: 'success',
    },
    dealerDefined: {
      // Preserve raw Dealer.com data
      raw,
      dealerCode: raw.dealerCode ?? raw.dealer_code,
    },
    enrichment: {
      // Store additional Dealer.com data if available
