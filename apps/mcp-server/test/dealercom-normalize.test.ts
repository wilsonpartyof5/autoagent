/**
 * Dealer.com Normalize Tests
 * 
 * Tests for Dealer.com provider mapper normalization to UVS.
 */

import { describe, it, expect } from 'vitest';
import { normalize } from '../src/ingestion/providers/dealerCom.js';
import { validateUVS } from '../src/validation/validateUVS.js';
import type { DealerComVehicle } from '../src/ingestion/providers/dealerCom.js';

describe('Dealer.com Normalize', () => {
  it('should normalize a minimal Dealer.com vehicle to valid UVS', () => {
    const raw: DealerComVehicle = {
      id: 'dc-test-1',
      vehicleId: 'dc-vehicle-1',
  ...
  it('should normalize a fully populated Dealer.com vehicle', () => {
    const raw: DealerComVehicle = {
      id: 'dc-test-2',
      vehicleId: 'dc-vehicle-2',
  ...
  it('should prefer sellingPrice over price', () => {
    const raw: DealerComVehicle = {
      id: 'dc-test-price',
      year: 2023,
      make: 'Test',
    ...
    const uvs = normalize(raw);
    // Should use selling price (preferred)
    expect(uvs.pricing.price).toBe(25000);
  });

  it('should use retail price when selling price not available', () => {
    const raw: DealerComVehicle = {
      id: 'dc-test-retail',
      year: 2023,
      make: 'Test',
  ...
  it('should handle various field name variations', () => {
    const raw: DealerComVehicle = {
      id: 'dc-test-3',
      vehicle_id: 'dc-vehicle-3',
      stock_number: 'STK-003', // snake_case
      year: 2021,
      make: 'Ford',
      model: 'F-150',
      body_type: 'Truck', // snake_case
      selling_price: 35000, // snake_case
  ...
  it('should create engine specs from numeric fields even without description', () => {
    const raw: DealerComVehicle = {
      id: 'dc-test-engine-numeric-only',
      year: 2020,
      make: 'Test',
      model: 'Test',
      price: 10000,
      dealerName: 'Test Dealer',
      // No engine description string
      cylinders: 6,
      horsepower: 280,
      displacement: 3.6,
    };
    ...
    const uvs = normalize(raw);
    // Should create engine object from numeric fields even without description
    expect(uvs.coreSpecs?.engine).toBeDefined();
    expect(uvs.coreSpecs?.engine?.cylinders).toBe(6);
    expect(uvs.coreSpecs?.engine?.horsepower).toBe(280);
    expect(uvs.coreSpecs?.engine?.displacement).toBe(3.6);
  });
